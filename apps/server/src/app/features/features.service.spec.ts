...
import { ProductFeature, FeatureStatus } from './entities/product-feature.entity';
...
describe('FeaturesService', () => {
  ...
  describe('update', () => {
    it('should update a feature status', async () => {
      mockRepository.findOne.mockResolvedValue(mockFeature);
      mockRepository.save.mockResolvedValue({ ...mockFeature, status: FeatureStatus.DONE });

      const result = await service.update(mockFeature.id, { status: FeatureStatus.DONE });
