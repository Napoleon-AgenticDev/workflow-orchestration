...
meta:
id: products-agent-alchemy-dev-features-angular-zod-rules-engine-implementation-guide-md
...
# Zod Rules Engine - Implementation Guide
...
This guide provides step-by-step instructions for implementing Zod Rules Engine in Angular applications. By following this guide, you'll build a production-ready implementation that follows best practices and Clean Architecture principles.
...
Create `src/app/rules/rule.interface.ts`:
...
Create `src/app/rules/rules-engine.service.ts`:
...
Create base rule class in `src/app/rules/base-rule.ts`:

```typescript
import { IRule, RuleContext } from './rule.interface';
...
export abstract class BaseRule implements IRule {
  ...
  abstract execute(context: RuleContext): boolean;

  protected setResult(isValid: boolean, message: string): boolean {
...
// Example rule
export class RequiredFieldRule extends BaseRule {
  ...
  execute(context: RuleContext): boolean {
    const value = context.data[this.fieldName];
    const isValid = value !== null && value !== undefined && value !== '';
    return this.setResult(
      isValid,
      isValid ? '' : `${this.fieldName} is required`
    );
  }
}
```
...
export class ExampleComponent implements OnInit {
  ...
  constructor(private service: ZodRulesEngineService) {}
...
```typescript
describe('ZodRulesEngineService', () => {
  let service: ZodRulesEngineService;
  let httpMock: HttpTestingController;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
      providers: [ZodRulesEngineService]
    });
    
    service = TestBed.inject(ZodRulesEngineService);
